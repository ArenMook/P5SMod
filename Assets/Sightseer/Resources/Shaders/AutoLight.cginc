...
// ---- Screen space direction light shadows helpers (any version)
        ...
        inline fixed unitySampleShadow (unityShadowCoord4 shadowCoord)
        {
                ...
                shadow = _LightShadowData.r + shadow * (1.0 - _LightShadowData.r);
                return shadow;
            #else
...
{
	half light = 1.0 - saturate(dot(lightCoord, lightCoord));
	light *= light * light;
	return light;
}
...
#define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
	...
	fixed destName = UnitySpotAttenuate(lightCoord) * shadow;
...
inline half UnitySpotCookie(unityShadowCoord4 lightCoord)
{
	//return tex2D(_LightTextureB0, dot(lightCoord, LightCoord).xx).UNITY_ATTEN_CHANNEL;
	half f = saturate(length(lightCoord.xy / lightCoord.w * 2.0));
	f *= f;
	f *= f;
	return 1.0 - f;
}
...
#define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
    ...
    fixed shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
    fixed destName = (lightCoord.z > 0.0) * UnitySpotCookie(lightCoord) * UnitySpotAttenuate(lightCoord.xyz) * shadow;
#endif
...
#define UNITY_LIGHT_ATTENUATION(destName, input, worldPos) \
    ...
    fixed shadow = UNITY_SHADOW_ATTENUATION(input, worldPos); \
	fixed destName = UnitySpotAttenuate(lightCoord) * shadow;
	//fixed destName = tex2D(_LightTextureB0, dot(lightCoord.xyz, lightCoord.xyz).rr).UNITY_ATTEN_CHANNEL * texCUBE(_LightTexture0, lightCoord).w * shadow;
#endif
