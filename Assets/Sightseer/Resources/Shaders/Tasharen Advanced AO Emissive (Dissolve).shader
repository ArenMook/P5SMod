Shader "Tasharen/Advanced AO Emissive (Dissolve)"
{
	Properties
	{
		...
		[Soace][NoScaleOffset] _GlowMap ("Glow Map (RGB), Tint Amount (A)", 2D) = "black" {}
	...
	SubShader
	{
		...
		sampler2D _DamageTex, _GlowMap;
		half4 _TriplanarScale, _Glow;
		half _Condition, _FadeOut, _GlowBrightness;
		...
		void surf (Input IN, inout Output o)
		{
			...
			o.Emission = tex2D(_GlowMap, IN.tc.xy).rgb * _Glow.rgb * (_Glow.a * _GlowBrightness);
